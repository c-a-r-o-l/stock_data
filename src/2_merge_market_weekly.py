"""
2_merge_market_weekly.py
========================
Merges weekly SPY and QQQ technicals on DATE.  No MKT data — market
internals are daily-only and don't make sense at weekly grain.

Inputs:
  data/History_2_W_clean/SPY.csv   — SPY weekly technicals (cleaned)
  data/History_2_W_clean/QQQ.csv   — QQQ weekly technicals (cleaned)

Output:
  data/History_6_merge/market_merge_weekly.csv
"""
...
def main():
    ...
    spy_path = project_root / "data/History_2_W_clean/SPY.csv"
    qqq_path = project_root / "data/History_2_W_clean/QQQ.csv"
    out_path = project_root / "data/History_6_merge/market_merge_weekly.csv"
    ...
    print(f"SPY (weekly): {len(spy_data)} dates, QQQ (weekly): {len(qqq_data)} dates")
